...
public class JLANServer implements ServerListener {
	...
	public static final int CheckPointCreateWebDavProvider = 13;
	...
	protected void start( String[] args) {
		...
		try {
			...
			//  Create the WebDav provide, if enabled
			if ( m_srvConfig.hasConfigSection( WEBDAVConfigSection.SectionName)) {
				checkPoint( out, CheckPointCreateWebDavProvider);
				m_srvConfig.addServer( createWebDavProvider( m_srvConfig));
			}
	...
	/**
	 * Create the Webdav provider
	 * 
	 * @param config ServerConfiguration
	 * @return NetworkServer
	 * @exception Exception
	 */
	protected final NetworkServer createWebDavProvider(ServerConfiguration config)
	throws Exception {
		return createServer( "org.alfresco.jlan.webdav.WebDavProvider", config);
	}
